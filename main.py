...
class AppState:
    ...
    # KÉRT MÉRETEK (NE VÁLTOZTASD MEG)
    WIN_WIDTH = 800
    WIN_HEIGHT = 800
    UI_HEIGHT = 300

# Inicializálás (GPU-n indítjuk)
try:
    ti.init(arch=ti.gpu)
except:
    ti.init(arch=ti.cpu)
...
def render_gpgpu_kernel(t: float):
    for i, j in pixels_gpu:
        if j < AppState.UI_HEIGHT:
            pixels_gpu[i, j] = ti.Vector([0.15, 0.15, 0.15]) # Háttér szín
        else:
...
def draw_button(gui, x, y, w, h, text, is_active, mouse_x, mouse_y, is_clicked):
    # Színek
    color_bg = 0x444444 # Szürke
    if is_active:
        color_bg = 0x00AA00 # Aktív Zöld
    ...
    if is_hover and not is_active:
        color_bg = 0x666666 # Világosabb szürke hovernél
...
def main():
    ...
    # --- UI LAYOUT (800x800 ablak, UI=300px) ---
    # UI arány = 300 / 800 = 0.375
    ui_top = AppState.UI_HEIGHT / AppState.WIN_HEIGHT

    # Slider pozíció (felül, Y=0.30)
    slider_y = 0.30
    slider_x = 0.5
    slider_w = 0.8
    is_dragging = False
    ...
    btn_w = 0.30
    btn_h = 0.06
    btn_row_1_y = 0.20 # Felső sor
    btn_row_2_y = 0.10 # Alsó sor
    ...
    while gui.running:
        ...
        # 1. Lépés: Kép kiszámolása és beállítása
        ...
        # --- 2. Lépés: UI KIRAJZOLÁS (Most már biztosan a kép tetejére kerül) ---

        should_check_buttons = is_mouse_down and not is_dragging

        # Gombok kirajzolása és logika
        # 1. SOR: GPGPU vs CPU
